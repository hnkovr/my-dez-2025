...
# Check for local dagster directory that may cause import conflicts
if [ -d "dagster" ]; then
    echo "⚠️ Warning: Local 'dagster' directory detected which may conflict with installed packages"
    echo "⚠️ Temporarily renaming it to 'dagster_local' to avoid import conflicts"
    mv dagster dagster_local
fi
...
# If we renamed the dagster directory, move it back
if [ -d "dagster_local" ]; then
    echo "📁 Restoring original 'dagster' directory"
    mv dagster_local dagster
    echo "ℹ️ Note: The local 'dagster' directory may conflict with imported packages."
    echo "ℹ️ Consider renaming this directory to avoid confusion with the installed package."
fi
